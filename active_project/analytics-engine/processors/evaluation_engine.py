...
def evaluate_causal_uplift(conn) -> List[Dict]:
    """
    Evaluates the causal uplift generated by the Contextual Bandit interventions.
    Uses Inverse Probability Weighting (IPW) to calculate the Conditional Average Treatment Effect (CATE).
    """
    log.info("Evaluating Causal Uplift from Bandit Interventions...")
    
    with conn.cursor() as cur:
        # We join with decision_contexts to get the ultimate purchase outcome.
        # But for now, we just use the `outcome` field if it's updated. 
        # Since outcome=1 means converted, we use that.
        cur.execute("""
            SELECT intervention, COUNT(*) as sample_size,
                   AVG(case when assigned_arm = 'NONE' then outcome else null end) as base_conversion,
                   AVG(case when assigned_arm != 'NONE' then outcome else null end) as treatment_conv,
                   COUNT(case when assigned_arm != 'NONE' then 1 else null end) as treatment_n,
                   COUNT(case when assigned_arm = 'NONE' then 1 else null end) as control_n
            FROM (
                SELECT i.assigned_arm as intervention, 
                       i.outcome, i.assigned_arm, i.propensity
                FROM intervention_logs i
            ) tbl
            GROUP BY intervention
            HAVING intervention != 'NONE'
        """)
        ...
        cur.execute("""
             ...
             WHERE propensity > 0 AND propensity < 1
        """)
        logs = cur.fetchall()

    if not logs or len(logs) < 10:
        log.warning("Not enough intervention logs for causal uplift evaluation. Skipping.")
        return []

    results = []
    
    # Calculate simple A/B stats + IPW
    ...
    for arm in arms:
        ...
        if t_mask.sum() < 5 or c_mask.sum() < 5:
            continue
        ...
        # E[ Y * I(T=1) / P(T=1|X) ] - E[ Y * I(T=0) / P(T=0|X) ]
        # Note: Propensity stored is P(Assigned Arm). For control, it's P(NONE).
        ipw_ate = 0
        weight_t = t_mask / np.maximum(propensities, 0.01)
        weight_c = c_mask / np.maximum(propensities, 0.01)
        
        if weight_t.sum() > 0 and weight_c.sum() > 0:
            ate_t = (y * weight_t).mean() / max(weight_t.mean(), 0.0001)
            ate_c = (y * weight_c).mean() / max(weight_c.mean(), 0.0001)
            ipw_ate = ate_t - ate_c
        
        # standard t-test for p_value
        ...
        results.append({
            'intervention': arm,
            'sample_size': t_mask.sum() + c_mask.sum(),
            ...
            'notes': f"Causal IPW ATE calculated vs NONE control arm. Raw Lift: {uplift_rel*100:.1f}%. p={pval:.4f}",
            'run_id': None
        })

    log.info(f"Evaluated Causal Uplift for {len(results)} interventions.")
    return results
