...
export default async function medusaRequest({
  ...
  const options: RequestInit = {
    ...
    headers: {
      ...
      // Medusa expects the publishable API key under this header name
...
const reshapeCart = (cart?: MedusaCart | null): Cart => {
  ...
  const cost = {
    totalAmount: {
      amount: convertToDecimal(
        lineItem.total,
        lineItem.variant && lineItem.variant.prices && lineItem.variant.prices[0]
          ? lineItem.variant.prices[0].currency_code
          : 'USD'
      ).toString(),
      currencyCode:
        (lineItem.variant && lineItem.variant.prices && lineItem.variant.prices[0]
          ? lineItem.variant.prices[0].currency_code?.toUpperCase()
          : 'EUR') || 'EUR'
    }
  };
...
const reshapeProduct = (product: MedusaProduct): Product => {
  ...
  // Centralize pricing logic via calculateVariantAmount so it matches cart behavior
  const { amount, currencyCode } = calculateVariantAmount(variant);
  ...
  const thumbMatch = product.thumbnail?.match(/.*\/(.*)\..*/);
  const featuredImageFilename = thumbMatch ? thumbMatch[1] : '';
  const featuredImage = {
...
export async function createCart(): Promise<Cart> {
  const res = await medusaRequest({
    method: 'POST',
    path: '/carts',
    payload: { region_id: 'reg_01KF8E5JQQEDBKAC8395HNP3X5' }
  });

  // reshapeCart now safely handles undefined/null carts and returns an "empty" cart shape
...
export async function getCategoryProducts(
  ...
  if (!res || !res.body.product_categories || res.body.product_categories.length === 0) {
    return [];
  }
...
export async function getProducts({
  ...
  // Debug logging to understand what the Medusa backend is returning.
  if (!res.body || !('products' in res.body)) {
    console.error('Medusa getProducts: `products` field missing in response body:', res.body);
  }
